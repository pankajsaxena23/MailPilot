...
def get_client(api_key=None):
    """
    Returns an initialized Gemini Client with the given API key or from environment variables.
    """
...
def _execute_with_model_fallback(client, prompt: str, preferred_model: str = None) -> tuple[str, str]:
    ...
    """
    configured_model = preferred_model or os.environ.get("GEMINI_MODEL", "gemini-2.5-flash")
    
    # Build candidate models list starting with the preferred/configured model
...
def test_ai_connection(api_key: str = None, model: str = None) -> dict:
    ...
    try:
        ...
        test_model = model or os.environ.get("GEMINI_MODEL", "gemini-2.5-flash")
